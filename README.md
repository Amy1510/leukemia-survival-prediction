...
├── models/                    # Not versioned, generated by the notebook (~3 GB)
...
- **Each bar**: the share of the gap attributable to one variable
  (positive contributions raise survival, negative contributions lower it).
- **Verification**: the equality above is SHAP's additivity property, checked on every
  prediction. When it holds, the decomposition is exact rather than approximate.
...
| Global ranking vs. log-rank | cytogenetics (ELN) ranks first, matching the univariate test |
...
| **KANE Amy** | Explainability (SHAP), bias analysis, i18n, EDA, preprocessing |
